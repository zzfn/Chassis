...
- **回合模型（队列延迟）**：`onIdle` 仅在命令队列为空时被调用；`onIdle` 填充队列后，**第一条命令在同一回合立即消费**，后续命令在之后的回合逐帧（每回合 1 条）消费
- **弹速**：2 tile/回合；**伤害**：25 HP/发；**射击冷却**：子弹命中/出界前不可再射（同时最多 1 颗子弹）；**最大回合**：300
...
5. 所有子弹前进 2 格（`BULLET_SPEED=2`），处理碰撞
6. 捡星星检测
7. 记录遥测帧 / 胜负判断
